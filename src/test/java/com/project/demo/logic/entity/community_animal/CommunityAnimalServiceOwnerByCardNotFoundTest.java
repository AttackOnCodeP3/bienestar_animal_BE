...
class CommunityAnimalServiceOwnerByCardNotFoundTest {
    ...
    @Test
    void whenOwnerIdentificationCardProvided_andNotFound_thenThrows() {
        CreateAnimalRequestDTO dto = new CreateAnimalRequestDTO();
        dto.setOwnerIdentificationCard("123-ABC");

        when(userRepository.findByIdentificationCard("123-ABC")).thenReturn(Optional.empty());

        var ex = assertThrows(IllegalArgumentException.class,
                () -> service.createCommunityAnimal("ignored@mail.com", dto));

        org.assertj.core.api.Assertions.assertThat(ex.getMessage()).contains("Propietario no encontrado");
    }
}
